...
First of these is the jewel string J and the second one is stone string S. You can assume that 1 <= T <= 100, 1 <= |J|, |S| <= 100

Output
Output for each test case, a single integer, the number of jewels mined.
"""
"""
1) check for every character in string S and string J.
2) character in string S lies in string J 
3) maintain a count and increses by 1
4) print the count
"""
T = int(input())
for t in range(T):
